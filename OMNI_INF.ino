...
boolean hasFoundSpencifiedBleDev(const LGATTDeviceInfo &info)
{
    // you can check your known remote device address here, or else the first descovered ble device will be connected.
            ...
            BluTag1.addr[1] = 0x35;
            BluTag1.addr[0] = 0xbd;
     if((info.bd_addr.addr[5]==BluTag1.addr[5])&&(info.bd_addr.addr[4]==BluTag1.addr[4])&&(info.bd_addr.addr[3]==BluTag1.addr[3])&&(info.bd_addr.addr[2]==BluTag1.addr[2])&&(info.bd_addr.addr[1]==BluTag1.addr[1])&&(info.bd_addr.addr[0]==BluTag1.addr[0]))
    {
       return true; 
    }
    return false;
...
void loop() 
{
    ...
    while (1)
    {
        ...
        for (i = 0; i < num; i++)
        {
            ...
            Serial.printf("len  %d\n", info.eir_len);
            for (int i = 0; i < info.eir_len ; i++){
              Serial.printf(" %x %x %x %x %x %x",info.bd_addr.addr[5],info.bd_addr.addr[4],info.bd_addr.addr[3],info.bd_addr.addr[2],info.bd_addr.addr[1],info.bd_addr.addr[0]);
            }
            Serial.println();
    ...
    while (1)
    {
      c.readRemoteRSSI(info.bd_addr,r);
      if(r>-35){
        //green
       bar.singleLed(1,LOW);
       bar.singleLed(2,LOW);
      ...
      level = 0;
      if(r<-75){
       bar.singleLed(1,HIGH);
       bar.singleLed(2,HIGH);
       bar.singleLed(3,HIGH);
       bar.singleLed(4,HIGH);
       bar.singleLed(5,HIGH);
       bar.singleLed(6,HIGH);
       bar.singleLed(7,HIGH);
       bar.singleLed(8,HIGH); 
       bar.singleLed(9,HIGH);
      }
      Serial.printf("%d\n",r);
      delay(1000);
